...
let totalSalesAmount = sales.reduce((acc, curr) => acc + curr.amount, 0)
...
//
// solution:
console.log("\nSolution to stage 2, q9: ")
employees = [
  { name: "Alice", dept: "Engineering", salary: 80000 },
  { name: "Bob", dept: "HR", salary: 50000 },
  { name: "Charlie", dept: "Engineering", salary: 95000 }
]
engineeringEmployees = employees.filter(emp => emp.dept === "Engineering") //filtering engineering employees
console.log(engineeringEmployees)
...
const aboveHundredItem = cart.filter(item => item.subtotal > 100)
...
// this reducing took lots of brainstorming. 
// i just couldnt do it and looked for hints
// uhh.. , yeah.. may be all of the code
frequency = antiBotLogs.reduce((acc, curr) => {
  if (!acc[curr.page])
    acc[curr.page] = 1
  else acc[curr.page] += 1
...
for (key in frequency) {
  let plural = (frequency[key] > 1) ? "s" : ""
  console.log(`Page: ${key} → ${frequency[key]} visit${plural}`)
}
